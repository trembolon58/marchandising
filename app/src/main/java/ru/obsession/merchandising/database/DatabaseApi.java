...
public class DatabaseApi {
    ...
    public void insertAll(ArrayList<Work> works,
        ...
        for (Goods goods1 : goods) {
            ...
            values.put("client_id", goods1.clientId);
    ...
    public ArrayList<Goods> getAssortment(int userId, int clientId, Shop shop, int date) {
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM goods" +
                ...
                " AND goods.date = " + String.valueOf(date) +
    ...
    public void saveReport(ArrayList<Goods> goodses, int userId, int clientId, Shop shop, int date) {
        db.beginTransaction();
        db.execSQL("DELETE FROM goods" +
                " WHERE goods.shop_id = " + shop.id +
                " AND goods.user_id = " + String.valueOf(userId) +
        ...
        for (Goods goods : goodses) {
            ...
            values.put("shop_id", shop.id);
            values.put("shop_name", shop.name.toUpperCase());
            ...
            values.put("return", goods.returned);
    ...
    private void parseAssortmenteCursor(ArrayList<Goods> goodses, Cursor cursor) {
        if (cursor.getCount() != 0) {
            while (cursor.moveToNext()) {
                ...
                goods.returned = cursor.getString(cursor.getColumnIndex("return"));
    ...
    public void removePhoto(Photo photo) {
        db.execSQL("DELETE FROM photos WHERE path = '" + photo.path + "'");
    }

    public void saveOrder(ArrayList<Goods> goodses, int userId, int clientId, Shop shop, int date) {
        db.beginTransaction();
        db.execSQL("DELETE FROM orders" +
                " WHERE orders.shop_id = " + String.valueOf(shop.id) +
                " AND orders.user_id = " + String.valueOf(userId) +
                " AND orders.date = " + String.valueOf(date) +
                " AND orders.client_id = " + String.valueOf(clientId));
        ContentValues values = new ContentValues();
        for (Goods goods : goodses) {
            values.put("need_order", goods.needOrder? 1 : 0);
            values.put("order_number", goods.orderNumber);
            values.put("date", date);
            values.put("user_id", userId);
            values.put("client_id", clientId);
            values.put("shop_id", shop.id);
            values.put("shop_name", shop.name);
            values.put("goods_id", goods.id);
    ...
    public ArrayList<Goods> getAssortmentOrder(int userId, int clientId, Shop shop, int date) {
        ArrayList<Goods> goodses = new ArrayList<Goods>();
        Cursor cursor = db.rawQuery("SELECT * FROM orders" +
                " INNER JOIN goods ON goods.goods_id = orders.goods_id" +
                " WHERE orders.shop_name = '" + shop.name + "'" +
                " AND orders.user_id = " + String.valueOf(userId) +
                " AND orders.date = " + String.valueOf(date) +
                " AND orders.shop_id = " + String.valueOf(shop.id) +
                " AND orders.client_id = " + String.valueOf(clientId +
        ...
        if (cursor.getCount() == 0) {
            ...
            while (cursor.moveToNext()) {
                ...
                goods.format = cursor.getString(cursor.getColumnIndex("format"));
                goods.cost = cursor.getString(cursor.getColumnIndex("cost"));
                goods.needOrder = cursor.getInt(cursor.getColumnIndex("need_order")) == 1;
                goods.orderNumber = cursor.getString(cursor.getColumnIndex("order_number"));
                goods.calcDescription();
                goodses.add(goods);
            }
        }
    ...
    public int getWorkTime(int userId, int clientId, Shop shop, int date) {
        int out = date;
        ...
        if (cursor.getCount() == 0) {
            while (cursor.moveToNext()) {
                out = cursor.getInt(cursor.getColumnIndex("date"));
            }
        }
        cursor.close();
        return out;
